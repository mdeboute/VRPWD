...
class VRPWDMIPModel_1:
    def __init__(self, instance: VRPWDData):
        ...
        self.road_time = {
            (a, b): instance.graph.edges[a, b]["travel_time"]
        ...
        self.demand = {
            node: instance.graph.nodes[node]["demand"] for node in self.nodes
        }
        ...
        def subtourelim(model, where):
            if where == GRB.Callback.MIPSOL:
                ...
                if not is_tour_complete(tour):
                    ...
                    nt_demand = sum(
                        [self.demand[i] for i in self.demand_nodes if i not in tour]
                    )
                ...
                while tovisit:
                    ...
                    for i, j in drone_covered:
                        ...
                        if j == current:
                            ...
                            current = i
                    neighbors = [
                        j for _, j in edges.select(current, "*") if j not in visited
                    ]
