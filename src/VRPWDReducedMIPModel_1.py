...
class VRPWDReducedMIPModel_1:
    ...
    def create_solution(self, tour: list, drone_covered: dict) -> dict:
        ...
        node_demands = {
            node: self.instance.graph.nodes[node]["demand"] for node in self.instance.dpd_nodes
        }
        for i, x in enumerate(truck_solution[:-1]):
            ...
            if x in node_demands.keys() and node_demands[x] > 0.0:
                ...
                for k in range(start_index, len(solution["truck"])):
                    if len(solution["truck"][k]) == 3 and solution["truck"][k][2] > value:
    ...
    def __init__(self, instance: VRPWDData):
        ...
        self.demand = {i : instance.graph.nodes[instance.dpd_nodes[i]]["demand"] for i in self.nodes}
        self.drone_time = {
            (i, j): instance.drone_time_matrix[instance.dpd_nodes[i]-1][instance.dpd_nodes[j]-1] 
            for i, j in permutations(self.nodes, 2) if self.demand[j] > 0
        }
        ...
        # Demand Constraints:
        self.model.addConstrs(self.demand[j] * self.x.sum("*", j) + self.y1.sum("*", j) + self.y2.sum("*", j) == self.demand[j] for j in self.nodes)
        ...
        self.model.addConstrs(self.T[i] >= 30 * self.y1.sum(i, "*") + 2 * gp.quicksum(self.drone_time[i,j] * self.y1[i,j] for j in self.nodes if j!=i and self.demand[j] > 0) for i in self.nodes)
        self.model.addConstrs(self.T[i] >= 30 * self.y2.sum(i, "*") + 2 * gp.quicksum(self.drone_time[i,j] * self.y2[i,j] for j in self.nodes if j!=i and self.demand[j] > 0) for i in self.nodes)
        self.model.addConstrs(self.T[i] >= 30 * ( self.y1.sum(i, "*") + self.y2.sum(i, "*") ) + 60 * self.x.sum("*", i) for i in self.nodes if self.demand[i]>0)
    ...
    def solve(
        ...
        def subtourelim(model, where):
            if where == GRB.Callback.MIPSOL:
                ...
                drone_covered = gp.tuplelist(
                    (i,j) for i,j in model._vars[1].keys() if drone1_vals[i, j]> 0.5 or drone2_vals[i, j] > 0.5
                )
                ...
                if len(tour) < len(self.nodes):
                    ...
                    nt_demand = sum([self.demand[i] for i in self.nodes if i not in tour])
                    tour_routes = gp.tuplelist(
                        (i, j) for i, j in model._vars[0].keys() if i in tour and j in tour
                        )
                    for a,b in tour_routes:
                        if 0 in tour:
                            ...
                            model.cbLazy(
                                 model._vars[0][a,b] <= 
                                 gp.quicksum(model._vars[0][i, j] for i,j in model._vars[0].keys() if i in tour and j not in tour)
                            )
        ...
        drone_vals = {(i,j): round(drone1_vals[i, j] + drone2_vals[i, j]) 
                      for i,j in drone1_vals.keys() if round(drone1_vals[i, j] + drone2_vals[i, j]) > 0}
        ...
        solution = self.create_solution(truck_tour, drone_covered)
