    ...
    def second_stage(self, dpd_nodes_per_vehicle):
        ...
        for src_dest in pair_by_pair_truck_dpd_nodes:
            ...
            for key in range(1,self.number_of_drones+1):
                print('key = ',key)
                print('drone {} target : {}'.format(key,dpd_nodes_per_vehicle[key][counter]))
                for current_truck_node in sp_truck: 
                    print('current_truck_node : ',current_truck_node)
                    sp_d=self.instance.drone_time_matrix[current_truck_node-1][dpd_nodes_per_vehicle[key][counter]-1]
                    print('sp_d = ',sp_d)
                    if sp_d<min_d_tt[key-1]:
                        print('better')
                        min_d_tt[key-1]=sp_d
                        min_d_node[key-1]=current_truck_node
                d_back_to_truck_time[key-1]=self.instance.drone_time_matrix[min_d_node[key-1]-1][sp_truck[-1]]
            #here the best nodes are founded
            for o,a,b in zip([i for i in range(1,self.number_of_drones+1)],min_d_node,min_d_tt):
                print('best d{} node : {} with {} s'.format(o,a,b))
            print('min_d_tt : ',min_d_tt)
            print('min_d_node',min_d_node)
            print('d_back_to_truck_time : ',d_back_to_truck_time)
            #create solution 
            self.create_solution(solution,sp_truck,min_d_node,min_d_tt,d_back_to_truck_time,dpd_nodes_per_vehicle,counter)
    def create_solution(self,solution,sp_truck,min_d_node,min_d_tt,d_back_to_truck_time,dpd_nodes_per_vehicle,counter):
        ...
        drone_reaching_cdest_time=[round(a+b,3) for a,b in zip(min_d_tt,d_back_to_truck_time)]
        ...
        for current_truck_node in sp_truck:
            ...
            if nb_drones_to_launch!=0:
                ...
                for i in indices:
                    ...
                    tt=nx.shortest_path_length(self.instance.graph,starting_node,current_truck_node,weight='travel_time')
                    ...
                    preparing_event=(current_truck_node,current_truck_node,preparing_drone_time,'d{}'.format(i))
                    ...
                    drone_move1_event=(current_truck_node,dpd_nodes_per_vehicle[i][counter],min_d_tt[i-1])
                    #create drone move to come back to the truck 
                    drone_move2_event=(dpd_nodes_per_vehicle[i][counter],truck_destination,d_back_to_truck_time[i-1])
                ...
                if current_truck_node == truck_destination:
                    ...
                    time_to_deliver_demand=delivering_truck_time*demand_dict[current_truck_node]
                    ...
                    delivery_event=(current_truck_node,current_truck_node,time_to_deliver_demand,demand_dict[current_truck_node])
                    tt_last_launching_to_dest_plus_deliver=solution['0'][-1][2]+time_to_deliver_demand
                    #add it to the truck solution
                    ...
                    print('tt_last_launching_to_dest_plus_deliver = ',tt_last_launching_to_dest_plus_deliver)
                    ...
                    if max_drone_time>tt_last_launching_to_dest_plus_deliver:
                        waiting_time=max_drone_time-tt_last_launching_to_dest_plus_deliver
                        waiting_truck_event=(sp_truck[-1],sp_truck[-1],round(waiting_time,3))
                        ...
                        for time in drone_reaching_cdest_time:
                            if time<tt_last_launching_to_dest_plus_deliver:
                                waiting_drone_time=tt_last_launching_to_dest_plus_deliver-time
                                waiting_drone_event=(current_truck_node,current_truck_node,waiting_drone_time)
                                solution[str(drone_reaching_cdest_time.index(time)+1)].append(waiting_drone_event)
        print(solution)
